...
if not topics.exists():
    ...
    for topic in topics:
        ...
        # Ensure an even split: 50% scenario-based, 50% non-scenario-based
        num_questions = 20
        num_scenario_questions = num_questions // 2  # Ensures at least half are scenario-based
        num_non_scenario_questions = num_questions - num_scenario_questions  # Remaining are non-scenario-based

        # Create scenario-based questions
        for _ in range(num_scenario_questions):
            scenario = random.choice(scenarios_for_topic)
            question = Question.objects.create(
                topic=topic,
                scenario=scenario,
                level_of_difficulty=random.choice(list(difficulties.values())),
        ...
        # Create non-scenario-based questions
        for _ in range(num_non_scenario_questions):
            question = Question.objects.create(
                topic=topic,
                scenario=None,
                level_of_difficulty=random.choice(list(difficulties.values())),
